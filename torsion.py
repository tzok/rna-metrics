...
def calculate_torsion_angle(p1, p2, p3, p4):
    ...
    # Calculate angle
    angle = np.arctan2(np.dot(np.cross(n1, n2), v2/np.linalg.norm(v2)), np.dot(n1, n2))
        ...
        for chain in model:
            ...
            for i, residue in enumerate(residues):
                ...
                if all(x is not None for x in [O3_prev, P, O5, C5]):
                    angles[res_id]['alpha'] = calculate_torsion_angle(O3_prev, P, O5, C5)
                ...
                if all(x is not None for x in [C4, C3, O3, P_next]):
                    angles[res_id]['epsilon'] = calculate_torsion_angle(C4, C3, O3, P_next)
                
                if all(x is not None for x in [C3, O3, P_next, O5_next]):
                    angles[res_id]['zeta'] = calculate_torsion_angle(C3, O3, P_next, O5_next)
